...
@Suppress(
    "KDocUnresolvedReference", "MemberVisibilityCanBePrivate", "unused", "UNUSED_VALUE",
    "PublicApiImplicitType", "PrivatePropertyName"
)
open class CircularSeekBar @JvmOverloads constructor(
    context: Context? = null,
    attrs: AttributeSet? = null,
    defStyleAttr: Int = android.R.attr.progressBarStyleLarge,
    defStyleRes: Int = android.R.attr.progressBarStyleLarge,
) : View(context, attrs, defStyleAttr) {

    private val DP_TO_PX_SCALE = resources.displayMetrics.density
     ...
     */
    var pointerStrokeWidth: Float = 0F
        set(width) {
            field = width
            initPaints()
            recalculateAll()
     ...
     * Holds the color value for `mPointerHaloPaint` before the `Paint` instance is created.
     */
    private var pointerHaloColorOnTouch = DEFAULT_POINTER_HALO_COLOR_ON_TOUCH
     ...
     */
    var pointerAlphaOnTouch = DEFAULT_POINTER_ALPHA_ON_TOUCH
        set(alpha) {
            if (alpha in 0..255) {
                field = alpha
            }
     ...
     */
    private var moveOutsideCircle = DEFAULT_MOVE_OUTSIDE_CIRCLE

    /**
     * Used for enabling/disabling the lock option for easier hitting of the 0 progress mark.
     */
    var isLockEnabled = DEFAULT_LOCK_ENABLED
    ...
    private fun initAttributes(attrArray: TypedArray) {
        ...
        pointerHaloColorOnTouch = attrArray.getColor(
            ...
            DEFAULT_POINTER_HALO_COLOR_ON_TOUCH
        )
        circleColor =
        ...
        pointerAlphaOnTouch = attrArray.getInt(
            ...
            DEFAULT_POINTER_ALPHA_ON_TOUCH
        )
        if (pointerAlphaOnTouch > 255 || pointerAlphaOnTouch < 0) {
            pointerAlphaOnTouch = DEFAULT_POINTER_ALPHA_ON_TOUCH
        }
    ...
    private fun initPaints() {
        ...
        if (!disableProgressGlow) {
            ...
            circleProgressGlowPaint.maskFilter = BlurMaskFilter(
                PROGRESS_GLOW_RADIUS_DP * DP_TO_PX_SCALE,
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val ableToGoNegative =
            isNegativeEnabled && abs(totalCircleDegrees - 360f) < SMALL_DEGREE_BIAS * 2
...
//            }
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val isHardwareAccelerated = isHardwareAccelerated && layerType != LAYER_TYPE_SOFTWARE
        ...
        val padding =
            (circleStrokeWidth / 2f).coerceAtLeast(pointerStrokeWidth / 2 + pointerHaloWidth + pointerHaloBorderWidth) +
                    if (hasGlowEffect) PROGRESS_GLOW_RADIUS_DP * DP_TO_PX_SCALE else 0f
        ...
        if (maintainEqualCircle) { // Applies regardless of how the values were determined
            val min = circleHeight.coerceAtMost(circleWidth)
            circleHeight = min
            circleWidth = min
        }
        recalculateAll()
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        val touchEventRadius =
            sqrt1(distanceX.toDouble().pow(2.0) + distanceY.toDouble().pow(2.0))
                .toFloat()
        val minimumTouchTarget =
            MIN_TOUCH_TARGET_DP * DP_TO_PX_SCALE // Convert minimum touch target into px
        ...
        val outerRadius =
            circleHeight.coerceAtLeast(circleWidth) + additionalRadius // Max outer radius of the circle, including the minimumTouchTarget or wheel width
        val innerRadius =
            circleHeight.coerceAtMost(circleWidth) - additionalRadius // Min inner radius of the circle, including the minimumTouchTarget or wheel width
        additionalRadius =
        ...
        var touchAngle: Float =
            (atan2(y.toDouble(), x.toDouble()) / Math.PI * 180 % 360).toFloat() // Verified
        ...
        var cwDistanceFromStart: Float = touchAngle - startAngle // Verified
        cwDistanceFromStart =
        ...
        var cwDistanceFromEnd: Float = touchAngle - endAngle // Verified
        cwDistanceFromEnd =
        ...
        when (event.action) {
            MotionEvent.ACTION_DOWN -> {
                ...
                if (touchEventRadius in innerRadius..outerRadius &&
                    ...
                    return false
                } else if (touchEventRadius in innerRadius..outerRadius) { // If the user is touching near the circle
    ...
    override fun onRestoreInstanceState(state: Parcelable) {
        ...
        val superState = if (isV_T) {
            savedState.getParcelable("PARENT", Parcelable::class.java)
        } else {
            @Suppress("DEPRECATION")
            savedState.getParcelable("PARENT")
        }
    ...
    companion object {
        ...
        private const val DEFAULT_POINTER_HALO_WIDTH = 0f //6f
        ...
        /*private const val DEFAULT_CIRCLE_COLOR = Color.TRANSPARENT
        private const val DEFAULT_CIRCLE_PROGRESS_COLOR = Color.TRANSPARENT
        private const val DEFAULT_POINTER_COLOR = Color.TRANSPARENT
        private const val DEFAULT_POINTER_HALO_COLOR = Color.TRANSPARENT
        private const val DEFAULT_POINTER_HALO_COLOR_ON_TOUCH = Color.TRANSPARENT*/
        private const val DEFAULT_CIRCLE_COLOR = Color.TRANSPARENT
        //Color.DKGRAY

        private val DEFAULT_CIRCLE_PROGRESS_COLOR = Color.argb(235, 255, 100, 54)
        private val DEFAULT_POINTER_COLOR = Color.argb(235, 125, 112, 254)
        private const val DEFAULT_POINTER_HALO_COLOR = Color.TRANSPARENT
        ...
        private const val DEFAULT_POINTER_ALPHA_ON_TOUCH = 100
        private const val DEFAULT_USE_CUSTOM_RADII = false
        private const val DEFAULT_MAINTAIN_EQUAL_CIRCLE = true
        private const val DEFAULT_MOVE_OUTSIDE_CIRCLE = true //false
        private const val DEFAULT_LOCK_ENABLED = false //true
